...
plen = LEN(prefix)
line = prefix + LEFT$(padfull, 81 - plen)
WRITE #path, line
NEXT i
PRINT #2, "writer: wrote 50 x81B records fast, holding open, time="; RIGHT$(DATE$,8)
holdt = RIGHT$(DATE$,8)
pausesecs = 0
200 t = RIGHT$(DATE$,8)
IF t = holdt THEN 200
...
CLOSE #path
PRINT #2, "writer: closed at "; RIGHT$(DATE$,8); " -- done, 50 x81B records, held open ~10s"
END
